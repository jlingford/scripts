...
# give job a unique name
JOBNAME="new_queries"
# set path to input fasta files
INPUTDIR="./fastainput/new_queries/"
# set THREADS usage
T=95

# step 0: place fasta files for clustering in ./fastainput directory
# step 1 (done): create sequenceDB out of fastas
echo "SETTING UP SEQUENCE DATABASES"
...
seq_ids=("0.30" "0.50" "0.70" "0.80")
# coverages=("0.70" "0.80" "0.90")
coverages=("0.80")
...
for i in $(seq 1 1 3); do
    if [[ $i -eq 1 ]]; then
        COVMODE=0
        CLUSTMODE=0
        echo "SETTING CLUSTERING: COV-MODE 0; CLUST-MODE 0"
    elif [[ $i -eq 2 ]]; then
        COVMODE=0
        CLUSTMODE=1
        echo "SETTING CLUSTERING: COV-MODE 0; CLUST-MODE 1"
    elif [[ $i -eq 3 ]]; then
        COVMODE=1
        CLUSTMODE=2
    ...
    for cov in "${coverages[@]}"; do
        ...
        tar -czvf ./outfiles/${JOBNAME}-clustering-covm${suffix3}-clustm${suffix4}-cov${suffix1}.tar.gz --exclude="*.gz" ./outfiles/*

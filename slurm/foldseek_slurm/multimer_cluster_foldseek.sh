...
foldseek createtsv \
    ./queryDB/${name} \
    ./clusterDB/${name} \
    ./resultDB/${name}/${name}_clustersRAW.tsv

table="./resultDB/${name}/${name}_clustersRAW.tsv"
newtable=./resultDB/${name}/${name}_cluster_STATS.tsv
newlist=./resultDB/${name}/${name}_clustrep_list.txt
clustermembers=./resultDB/${name}/${name}_clusterrep_members.tsv
# make new table showing all cluster reps and their sizes... data for barplot
cut -f1 $table |
    uniq -c |
    sort -k1,1gr |
    sed 's/_model_0//' |
    awk -vOFS="\t" '{print $2, $1}' >$newtable
# make just a list of all cluster reps
cut -f1 $newtable |
    sed 's/_AFformat.*$//' >$newlist
# make table of cluster reps and their corresponding members
awk '{print $1, $2}' $newtable |
    sed 's/_AFformat_Boltz2_niFeLSUSSU_model_0_[AB]//g' |
    awk -vOFS="\t" '{$1=$1}1' >$clustermembers
...
foldseek convertalis \
    ...
    --format-mode 0 \
    --format-output query,target,evalue,pident,alntmscore,qtmscore,ttmscore,u,t,rmsd,lddt,prob,qcov,tcov,alnlen,qlen,tlen,qseq,tseq
...
# done <$newlist
find $INPUT -type f -name "*cif" |
    grep -f $newlist |
    rsync -a --no-relative --files-from=- . ./resultDB/${name}/ClusterrepPDBs_${name}
